...
const getPatientById = async (req, res) => {
  try {
    ...
    const [appointmentsResult, prescriptionsResult, reportsResult] =
      await Promise.all([
        supabaseAdmin
          .from("appointments")
          .select("*")
...
const createPrescription = async (req, res) => {
  try {
    ...
    logger.info(
      `Prescription created by doctor ${req.user.id} for patient ${patient_id}`,
    );
    res.status(201).json({
...
const updateDoctorProfile = async (req, res) => {
  try {
    ...
    if (consultation_fee !== undefined)
      doctorUpdate.consultation_fee = consultation_fee;
