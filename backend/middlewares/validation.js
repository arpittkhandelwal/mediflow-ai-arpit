...
const patientUpdateValidator = [
  body('age').optional().isInt({ min: 0, max: 150 }).withMessage('Valid age required'),
...
const symptomValidator = [
  body('symptoms').trim().notEmpty().withMessage('Symptoms description required')
    .isLength({ min: 10, max: 2000 }).withMessage('Symptoms must be 10-2000 characters'),
