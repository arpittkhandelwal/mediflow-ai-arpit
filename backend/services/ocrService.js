...
const basicParseMedicines = (text) => {
  ...
  const medicinePattern = /(?:tab\.?|cap\.?|inj\.?|syp\.?|drops?\.?|oint\.?)?\s*([A-Za-z]+(?:\s+[A-Za-z]+)?)\s+(\d+(?:\.\d+)?(?:mg|ml|mcg|g|IU)?)/gi;
  ...
  return medicines.length > 0 ? medicines : lines.map(l => ({ raw_line: l, name: l }));
};
